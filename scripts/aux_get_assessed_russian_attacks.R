# Get % of Ukraine subject of attacks by Russia:
library(sf)
library(tidyverse)
library(units)
library(readr)
...
for(i in dir('source-data/Ukraine_Conflict_Mapping_ARCGIS_Shapefiles/Russian_Advances_Shapefile/')){
  ...
  zipF <- paste0("source-data/Ukraine_Conflict_Mapping_ARCGIS_Shapefiles/Russian_Advances_Shapefile/", i)
  ...
  # Calculate area
  area_assessed_as_russia_attacks <- sum(st_area(temp))
  ...
  # Save
  res <- rbind(res, data.frame(area_assessed_as_russia_attacks, name = i))
}
...
res$date <- gsub('NO', 'NOV', res$date)
res$date <- gsub('AU', 'AUG', res$date)
...
# Calculate change in area assessed as Russia attacks
res <- res[order(res$date), ]
res$change_in_area_assessed_as_russia_attacks <- as.numeric(res$area_assessed_as_russia_attacks)-as.numeric(c(NA, res$area_assessed_as_russia_attacks)[1:nrow(res)])

res$change_in_area_assessed_as_russia_attacks_in_past_7_days <- NA
res$change_in_area_assessed_as_russia_attacks_in_past_30_days <- NA

for(i in 1:nrow(res)){
  res$change_in_area_assessed_as_russia_attacks_in_past_7_days[i] <- sum(res$change_in_area_assessed_as_russia_attacks[max(c(1, i-6)):i], na.rm = T)
  res$change_in_area_assessed_as_russia_attacks_in_past_30_days[i] <- sum(res$change_in_area_assessed_as_russia_attacks[max(c(1, i-29)):i], na.rm = T)
}

# Get this in %
res$area_assessed_as_russia_attacks_percent <- res$area_assessed_as_russia_attacks / (1000*1000*603628)
write_csv(res, "output-data/area_attacked_by_russia.csv")

# Generate maximal extent of Russian-assessed attacks (i.e. "ever" attacks by Russia since war began):
big_shp_combined <- st_union(big_shp)
saveRDS(big_shp_combined, 'output-data/area_ever_attacked_by_russia.RDS')
saveRDS(big_shp, 'output-data/area_attacked_by_russia_by_day.RDS')
