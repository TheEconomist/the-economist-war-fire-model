# Get % claimed as controlled by Russia (note: this is not the same as the territory assessed as controlled by Russia):
library(sf)
library(tidyverse)
library(units)
library(readr)
...
for(i in dir('source-data/Ukraine_Conflict_Mapping_ARCGIS_Shapefiles/Russian_Claimed_CoT/')){
  ...
  zipF <- paste0("source-data/Ukraine_Conflict_Mapping_ARCGIS_Shapefiles/Russian_Claimed_CoT/", i)
  ...
  if(is.na(st_crs(temp))){
    st_crs(temp) <- def_crs
  }
  temp <- temp %>% st_transform("EPSG:6381")

  # Calculate area
  area_claimed_as_russia_controlled <- sum(st_area(temp))
  ...
  # Save
  res <- rbind(res, data.frame(area_claimed_as_russia_controlled, name = i))
}

# Next, get the date based on the folder name:
res$date <- NA
for(i in 1:nrow(res)){
  res$date[i] <-  unlist(strsplit(strsplit(res$name[i], "_AO_")[[1]][2], '.zip'))
  if(is.na(res$date[i])){
    res$date[i] <- unlist(strsplit(strsplit(res$name[i], "AO")[[1]][2], '.zip'))
  }
  if(is.na(res$date[i])){
    res$date[i] <- unlist(strsplit(strsplit(res$name[i], "MapA")[[1]][2], '.zip'))
  }
  if(is.na(res$date[i])){
    res$date[i] <- unlist(strsplit(strsplit(res$name[i], "Ukraine")[[1]][2], '.zip'))
  }
  if(is.na(res$date[i])){
    res$date[i] <- unlist(strsplit(strsplit(res$name[i], "Ukraine_")[[1]][2], '.zip'))
  }
}
...
# Calculate change in area claimed as Russia controlled
res <- res[order(res$date), ]
res$change_in_area_claimed_as_russia_controlled <- as.numeric(res$area_claimed_as_russia_controlled)-as.numeric(c(NA, res$area_claimed_as_russia_controlled)[1:nrow(res)])

res$change_in_area_claimed_as_russia_controlled_in_past_7_days <- NA
res$change_in_area_claimed_as_russia_controlled_in_past_30_days <- NA

for(i in 1:nrow(res)){
  res$change_in_area_claimed_as_russia_controlled_in_past_7_days[i] <- sum(res$change_in_area_claimed_as_russia_controlled[max(c(1, i-6)):i], na.rm = T)
  res$change_in_area_claimed_as_russia_controlled_in_past_30_days[i] <- sum(res$change_in_area_claimed_as_russia_controlled[max(c(1, i-29)):i], na.rm = T)
}

# Get this in %
res$area_claimed_as_russia_controlled_percent <- res$area_claimed_as_russia_controlled / (1000*1000*603628)
write_csv(res, "output-data/area_claimed_as_controlled_by_Russia.csv")

# Generate maximal extent of Russian-claimed control (i.e. "ever" controlled by Russia since war began):
big_shp_combined <- st_union(big_shp)
saveRDS(big_shp_combined, 'output-data/area_ever_claimed_as_controlled_by_Russia.RDS')
saveRDS(big_shp, 'output-data/area_claimed_as_controlled_by_Russia_by_day.RDS')
